...
dsc  <-  function(gamma_grid,
  ...
  # Check if ew_grid is Integer vector with with values 0 and / or 1
  checkmate::assertIntegerish(ew_grid,
                             lower = 0,
                             upper = 1,
                             min.len = 1,
                             any.missing = FALSE)
  ...
  n_combs         <-  length(gamma_grid) * length(psi_grid) * length(ew_grid)
  parameter_grid  <-  vector(mode = "list", length = n_combs)
  index           <-  1L
  for (i in seq_along(gamma_grid)) {
    for (j in seq_along(psi_grid))  {
      for (k in seq_along(ew_grid))  {
        parameter_grid[[index]]  <-  c(i, j, k)
                        index    <-  index + 1
      }
    }
  }
  ...
  for (i in as.integer(seq_along(parameter_grid))) {
        ...
        # Create Model Name
        mod_name  <-  paste("gamma", gam, "psi", psi, "ew", ew, sep = "_")
  ...
  chosen_parameters[cbind(seq_len(len), max.col(cum_ln_scores_lag, "first"))]  <-  TRUE #nolint
