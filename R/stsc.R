...
#' vast sets of predictive signals, many of which may be irrelevant or short-lived.
#' This method transforms heterogeneous scalar-valued signals into
...
#' of the candidate forecasting models. The values of this tuning parameter are chosen
#' in a procedure that amounts to leave-one-out cross-validation,
...
#' the (possible) sizes of the subsets. The values of this tuning parameter are chosen
#' in a procedure that amounts to leave-one-out cross-validation,
...
#' `metric = 4` uses discounted Compounded-Returns (in this case the target variable
#' y has to be a time series of financial returns).
...
#' equal weights to combine the candidate forecasting models.
...
#' includes all candidate forecasting models generated by the first and third signals.
...
#'  \describe{
#'   \item{risk_aversion}{A non-negative double representing the investor's risk aversion.
#'                        Higher values indicate more risk-averse behavior.}
#'   \item{min_weight}{A double specifying the minimum weight allocated to the market.
#'                     A non-negative lower bound effectively rules out short sales.}
#'   \item{max_weight}{A double specifying the maximum weight allocated to the market.
...
#'     \item{Point_Forecast}{A vector with the first moments of the aggregate predictive densities of the STSC model.}
#'     \item{Variance_Prediction}{A vector with the second moments of the aggregate predictive densities of the STSC model.}
#'   }
#' }
#' \item{Tuning_Parameters}{A list containing:
#'   \describe{
#'     \item{Gamma}{A vector containing the selected values for the tuning parameter gamma.}
#'     \item{Psi}{A vector containing the selected values for the tuning parameter psi.}
#'     \item{Signals}{A matrix containing the selected signals.}
#'     \item{Lambda}{A matrix containing the selected values for the tuning parameter lambda.}
#'     \item{Kappa}{A matrix containing the selected values for the tuning parameter kappa.}
#'   }
#' }
...
#'    plot_cssed <- ggplot(data = data.frame(eval_period, cssed), aes(x = eval_period, y = cssed)) +
#'      geom_line() +
#'      ylim(-0.0008, 0.0008) +
