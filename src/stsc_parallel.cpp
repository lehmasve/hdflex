...
// Function VII - Loop over Gamma and Psi
   ...
   // Parallel Loop over gammas
      ...
      // Compute Maximum Set of Active Candidate Models
         active_models = dsc_active_models_par_(score_cands(g), psi_max);
      ...
      // Add Keep-Index & Remove Duplicates
         if (incl_idx.n_elem > 0) {
            ...
            active_idx = remove_duplicates_par(active_idx, incl_idx);
         }
      ...
      // Update score for Candidate Models
         dsc_score_cands_par_(score_cands(g), 
                              y_t, 
                              forecast_tvc_t,             
                              variance_tvc_t,
                              gamma,
   ...
   // Select Aggregated Forecast
      stsc_results = rank_comb_par_(score_combs, 
                                    forecasts_comb,
                                    variances_comb);
   ...
   // Update score for Combinations (Aggregated Predictive Distributions)
      dsc_score_comb_par_(score_combs, 
                          y_t, 
                          forecasts_comb,             
                          variances_comb,
                          delta,
...
// [[Rcpp::export]]
   ...
   // Loop over t
      ...
      // Apply DSC-Function
         ...
         dsc_results = dsc_loop_par_(score_cands,
                                     score_combs, 
                                     gamma_grid, 
                                     psi_grid, 
                                     y_pred,
