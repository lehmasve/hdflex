...
// [[Rcpp::export]]
  ...
  // Initialize - System Covariance
     var_y   = arma::var(y_var.elem(index));
     cov_mat = arma::zeros<arma::mat>(2, 2);
     cov_mat(0, 0) = 0;
     cov_mat(1, 1) = 0; 
...
// Function II a - Predictive and Update Step for time t ('Raw' Predictor Time Series)
// [[Rcpp::export]]
...
// Function II b - Predictive and Update Step for time t (Point Forecast Time Series)
// [[Rcpp::export]]
   List tvc_model_forecasts(arma::vec& y_var, 
                            arma::vec& x_var, 
                            int i, 
                            double kappa, 
                            arma::mat theta, 
  ...
  // Update H for Time t (Equation 10)
     double z_times_theta = arma::as_scalar(z_t * theta);
     h_new = arma::as_scalar(kappa * h + (1 - kappa) * pow(y_t - z_times_theta, 2));
      
  // Get Predictive Density for Predicting t + 1 (Equation 9)
     mu = arma::as_scalar(z_pred * theta);
     variance = arma::as_scalar(h_new + z_pred * cov_mat * z_pred.t());
...
// Function III a - Loop over t ('Raw' Predictor Time Series)
// [[Rcpp::export]]
...
// Function III b - Loop over t (Point Forecast Time Series)
// [[Rcpp::export]]
   List tvc_model_loop_forecasts(arma::vec& y_var, 
                                 arma::vec& x_var, 
                                 double kappa, 
                                 arma::mat theta, 
                                 arma::mat cov_mat, 
  ...
  // Start loop
     for (int i = 0; i < ts_length; i++) {
       ...
       tvc_results = tvc_model_forecasts(y_var,
                                         x_var,
                                         i + 1, 
                                         kappa,
                                         theta ,
   ...
   // Save Predictive Density for time t + 1
       mu_mat(drop_length + i + 1)       = tvc_results(1); 
       variance_mat(drop_length + i + 1) = tvc_results(2);  
     }
